...
# Install base, API, offline, and observability (Langfuse) extras without the project to improve caching
...
# And ensure pip is available for runtime installs (includes observability/Langfuse)
RUN --mount=type=cache,target=/root/.local/share/uv \
    uv sync --frozen --no-dev --extra api --extra offline --extra observability --no-editable \
    && /app/.venv/bin/python -m ensurepip --upgrade
